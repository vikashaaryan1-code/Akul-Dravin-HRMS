...
export class EmployeeService {
  ...
  async getStats(tenantId?: string): Promise<any> {
    ...
    // reducing DB load and latency by 75% for this stats retrieval.
    ...
    if (tenantId) {
      // Note: although the entity defines companyId, the existing code uses tenantId in queries
      query.where('employee.tenantId = :tenantId', { tenantId });
    }

    const rawStats = await query.getRawOne();
    return {
      total: parseInt(rawStats.total, 10) || 0,
      active: parseInt(rawStats.active, 10) || 0,
      inactive: parseInt(rawStats.inactive, 10) || 0,
      onLeave: parseInt(rawStats.onLeave, 10) || 0,
    };
  }
}
