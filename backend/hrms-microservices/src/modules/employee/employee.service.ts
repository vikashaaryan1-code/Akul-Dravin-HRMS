...
export class EmployeeService {
  ...
  async getStats(tenantId?: string): Promise<any> {
    // BOLT OPTIMIZATION: Consolidate 4 database counts into 1 single query using conditional aggregation.
    // This reduces database roundtrips and improves response time for the dashboard.
    const query = this.employeeRepository.createQueryBuilder('employee');

    if (tenantId) {
      // Mapping the incoming 'tenantId' to the 'companyId' property defined in Employee entity.
