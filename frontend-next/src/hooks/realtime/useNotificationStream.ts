...
/* ── Notification event DTO ──────────────────────────────────────────────────── */ export type NotificationEvent = {
 id: string;
...
/* ── Connection state ────────────────────────────────────────────────────────── */ export type WsStatus = 'connecting' | 'connected' | 'reconnecting' | 'disconnected' | 'error';

/* ── Hook ────────────────────────────────────────────────────────────────────── */ const WS_BASE = process.env.NEXT_PUBLIC_WS_URL ?? 'ws://localhost:3001';
const MAX_RETRIES = 5;
...
export function useNotificationStream(options: {
 ...
 /* Unexpected close — schedule reconnect */ const delay = Math.min(1000 * 2 ** retryCountRef.current, 30_000);
...
/* ── BullMQ queue telemetry stream ───────────────────────────────────────────── */ export type QueueEvent = {
 queue: string;
 jobId: string;
...
export function useQueueTelemetry(options: {
 ...
 }, [enabled, queues?.join(',')]); /* eslint-disable-line react-hooks/exhaustive-deps */ return { events };
}
