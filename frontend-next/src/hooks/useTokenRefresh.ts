...
function msUntilRefresh(token: string): number {
  ...
  // Refresh 60 seconds before expiry (minimum 5s lead time)
  return Math.max(0, expiresInMs - 60_000);
}
...
export function useTokenRefresh() {
  ...
  const doRefresh = useCallback(async (token: string) => {
    ...
    try {
      ...
      // Update store with new access token (preserve user object)
      if (user) {
        setSession({ accessToken: data.accessToken, user });
      }
      syncAuthCookie(data.accessToken);
    } catch {
      // Network error — don't log out, will retry on next visibility change
    } finally {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible' && accessToken) {
        ...
        const isNearExpiry    = (payload.exp * 1000 - Date.now()) < 120_000; // 2 min
