...
export function middleware(request: NextRequest) {
 ...
 /* Always allow public routes through */ if (isPublicRoute(pathname)) {
 return NextResponse.next();
 }

 /* Read the auth cookie (set by the client after login/OAuth) */ const token = request.cookies.get('akul-auth-token')?.value;

 if (!token) {
 /* No token → redirect to login with the original URL as ?next= param */ const loginUrl = new URL('/login', request.url);
 loginUrl.searchParams.set('next', pathname);
 return NextResponse.redirect(loginUrl);
 }

 // Basic JWT structure validation (Edge can't verify signature — just check format)
 /* Full verification happens on every API call via JwtAuthGuard on the backend. */ const parts = token.split('.');
 if (parts.length !== 3) {
 ...
 /* Token expired — redirect to login */ if (payload.exp && payload.exp * 1000 < Date.now()) {
 const loginUrl = new URL('/login', request.url);
 loginUrl.searchParams.set('next', pathname);
 const response = NextResponse.redirect(loginUrl);
 response.cookies.delete('akul-auth-token');
 ...
 /* Super-admin routes — require super_admin role */ if (isSuperAdminRoute(pathname)) {
 ...
 /* Malformed payload — redirect to login */ const loginUrl = new URL('/login', request.url);
 const response = NextResponse.redirect(loginUrl);
 response.cookies.delete('akul-auth-token');
 return response;
 }

 /* Add White Label routing headers */ const hostname = request.headers.get('host') || '';
