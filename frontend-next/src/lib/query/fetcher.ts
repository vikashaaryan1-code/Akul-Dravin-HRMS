...
export async function apiFetch<T>(path: string, options: FetchOptions = {}): Promise<T> {
 ...
 /* Auth token from Zustand store (safe to call outside React — it's a store subscription) */ const token = useAuthStore.getState().token;
 ...
 /* Detect JSON vs text responses */ const contentType = response.headers.get('content-type') ?? '';
