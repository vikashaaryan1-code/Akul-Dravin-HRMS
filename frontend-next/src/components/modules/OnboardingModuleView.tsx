...
export function OnboardingModuleView() {
 ...
 /* Simulate onboarding: newest 5 employees are "in onboarding" */ const sortedNew = [...employees].sort((a, b) => new Date(b.createdAt ?? 0).getTime() - new Date(a.createdAt ?? 0).getTime()).slice(0, 8);
 ...
 <p className="text-sm font-semibold text-slate-800 ">Onboarding Pipeline</p>
 ...
 <div className="py-12 text-center text-slate-500 text-sm">No employees in onboarding. Start by adding new hires.</div>
 ) : (
 <div className="space-y-3">
 {onboardingList.map((entry) => (
 <div key={entry.employeeId} className="flex items-center gap-4 p-4 rounded-xl border border-slate-100 bg-white/60 hover:bg-slate-50 transition">
 ...
 <span className="text-sm font-medium text-slate-800 truncate">{entry.name}</span>
 <span className="text-xs text-slate-500 shrink-0 ml-2">{entry.progress}%</span>
 </div>
 ...
 {/* Step guide */}
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-3">Onboarding Checklist</p>
 <div className="flex flex-col gap-2">
 {ONBOARDING_STEPS.map((step, i) => (
 <div key={step.id} className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 ">
 <span className={`p-2 rounded-lg ${step.color}`}>{step.icon}</span>
 <span className="text-sm text-slate-700 ">{step.label}</span>
 <span className="ml-auto text-xs text-slate-500">Step {i + 1}</span>
 </div>
 ))}
 </div>
 </GlassCard>
