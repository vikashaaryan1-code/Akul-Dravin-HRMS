  ...
  return (
    <div className="space-y-6 animate-rise">
      ...
      <div className="flex justify-center">
        <div className="flex items-center gap-1 p-1 rounded-xl bg-slate-100 dark:bg-slate-800 text-sm">
          {(['monthly', 'annual'] as const).map((b) => (
            <button key={b} onClick={() => setBilling(b)}
              className={`px-4 py-1.5 rounded-lg font-medium transition capitalize ${billing === b ? 'bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 shadow-sm' : 'text-slate-500'}`}>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {PLANS.map((plan) => {
          ...
          return (
            <div key={plan.id} className={`relative rounded-2xl border overflow-hidden transition ${plan.popular ? 'border-blue-500 shadow-holo-lg' : 'border-slate-200 dark:border-slate-700'}`}>
              {plan.popular && (
