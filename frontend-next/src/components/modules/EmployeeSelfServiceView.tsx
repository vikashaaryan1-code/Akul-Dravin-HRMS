...
/* ─── Mini Components ────────────────────────────────────────────────────────── */ function QuickStat({ icon, label, value, color }: {
...
function LeaveBar({ balance }: { balance: LeaveBalance }) {
 ...
 <span className="text-xs text-slate-600">{balance.type}</span>
 <span className="text-xs text-slate-500">{balance.available} days left</span>
 </div>
...
function GoalRow({ goal }: { goal: Goal }) {
 ...
 <p className="text-xs font-medium text-slate-700 leading-snug">{goal.title}</p>
...
/* ─── Attendance Clock Widget ────────────────────────────────────────────────── */ function AttendanceClock() {
 ...
 /* 2. Get IP (Simple stub or via service) */ const ipResponse = await fetch('https://api.ipify.org?format=json').catch(() => null);
 ...
 ${checkedIn ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700/50 text-slate-500'}`}>
...
/* ─── Main Component ─────────────────────────────────────────────────────────── */ const LEAVE_BALANCES: LeaveBalance[] = [
...
export function EmployeeSelfServiceView() {
 ...
 <p className="text-[10px] text-slate-500 text-center leading-tight">{ql.label}</p>
 </div>
 </Link>
 ))}
 </div>
 ...
 <p className="text-sm font-semibold text-slate-700">My Stats</p>
 ...
 <GlassCard className="space-y-3">
 <div className="flex items-center justify-between">
 <p className="text-sm font-semibold text-slate-700">My Goals (Q2)</p>
 ...
 <GlassCard className="space-y-3">
 <div className="flex items-center justify-between">
 <p className="text-sm font-semibold text-slate-700">Recent Payslips</p>
 ...
 <div>
 <p className="text-xs font-semibold text-slate-700">{ps.period}</p>
 ...
 <p className="text-sm font-semibold text-slate-700">Announcements</p>
 </div>
 <div className="space-y-2">
 {[
