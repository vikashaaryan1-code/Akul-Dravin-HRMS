...
/* ───────────────────────────────────────────────────────────────────────────── */ export function RecruiterHubModuleView() {
 ...
 <div>
 <p className="font-semibold text-slate-700 text-sm">{v as string}</p>
 ...
 <span className="font-semibold text-slate-600">{fmt(v as number)}</span>
 )},
 ...
 <div>
 <p className="text-[10px] uppercase tracking-[0.13em] text-slate-500">{s.label}</p>
 <p className="mt-2 text-2xl font-bold" style={{ color: s.accent }}>{s.value}</p>
 </div>
 <span className="p-2 rounded-xl bg-slate-50/80">{s.icon}</span>
 </div>
 </GlassCard>
 ))}
 </section>
 ...
 <p className="text-lg font-bold text-slate-700">{count} recruiter{count !== 1 ? 's' : ''}</p>
