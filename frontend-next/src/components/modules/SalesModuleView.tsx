...
export function SalesModuleView() {
  ...
  const { data: liveData, isLive, loading, error } = useApiResource({
    loader: async () => {
      const [summary, team, apiLeads, apiDeals, apiTargets, apiCommissions] = await Promise.all([
      ...
      const mappedTargets: SalesTargetRecord[] = apiTargets.map((target) => {
        ...
        return {
          id: target.id,
          ownerName: target.employeeId ? `Rep ${target.employeeId.slice(0, 6)}` : 'Sales Team',
      ...
      const mappedCommissions: SalesCommissionRecord[] = apiCommissions.map((commission) => ({
        id: commission.id,
        employeeName: `Rep ${commission.employeeId.slice(0, 6)}`,
      ...
      const mappedTeam: SalesTeamPerformanceRecord[] = team.map((item) => {
        const employeeName = `Rep ${item.employeeId.slice(0, 6)}`;
        return {
          id: item.employeeId,
          employeeName,
  ...
  const activeTeamPerformance = isLive && liveData.teamPerformance.length > 0 ? liveData.teamPerformance : salesTeamPerformance;
    ...
    <div className="space-y-5">
          ...
          <div className="flex flex-col items-start gap-2">
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-slate-400 disabled:cursor-not-allowed disabled:opacity-45 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-200"
            ...
            {!canImportLeads || !canCreateDeal ? (
              <p className="text-[11px] text-amber-700 dark:text-amber-300">Sales actions are restricted by active role policy.</p>
            ) : null}
          </div>
        }
      />
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <GlassCard>
          ...
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Lead ownership synced with employee master and role access.</p>
        </GlassCard>
        <GlassCard>
          ...
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Commission automation feeds bonus-ready payroll batches.</p>
        </GlassCard>
        <GlassCard>
          ...
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Hiring pipeline contributes pre-qualified BDR and AE candidates.</p>
        </GlassCard>
        <GlassCard>
          ...
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Revenue and win-rate projections flow into enterprise analytics.</p>
        </GlassCard>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <GlassCard className="space-y-3 p-4 sm:p-5">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Drag-and-Drop Sales Pipeline</p>
          ...
          {!canMovePipeline ? (
            <p className="text-xs text-amber-700 dark:text-amber-300">Your role has view-only pipeline access.</p>
          ) : null}
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {pipelineColumns.map((column) => (
              <div
                ...
                className="min-h-36 rounded-2xl border border-slate-200/70 bg-white/80 p-3 dark:border-slate-700/70 dark:bg-slate-900/70"
              >
                ...
                <div className="space-y-2">
                  {column.leads.map((lead) => (
                    <article
                      ...
                      className={`rounded-xl border border-slate-200/80 bg-white p-3 shadow-sm transition dark:border-slate-700 dark:bg-slate-900 ${
                    ...
                    >
                      ...
                      <div className="mt-2 flex items-center justify-between text-xs">
                        ...
                        <span className="font-semibold text-slate-700 dark:text-slate-200">{formatCurrency(lead.expectedValue)}</span>
                      </div>
                    </article>
                  ))}
                </div>
        ...
        <div className="space-y-4">
          ...
          <GlassCard>
            <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Lead Nurturing Automation</p>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
        <GlassCard className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Lead Management</p>
            <div className="flex flex-wrap gap-2">
              <input
                value={searchQuery}
                onChange={(event) => setSearchQuery(event.target.value)}
                placeholder="Search lead, company, owner"
                className="h-9 w-56 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-aqua dark:border-slate-700 dark:bg-slate-900"
              />
              <select
                value={stageFilter}
                onChange={(event) => setStageFilter(event.target.value as typeof stageFilter)}
                className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-sm outline-none transition focus:border-aqua dark:border-slate-700 dark:bg-slate-900"
              >
        ...
        <GlassCard className="space-y-3">
          <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Customer CRM</p>
          <div className="space-y-3">
            {salesCustomerAccounts.map((account) => (
              <div
                key={account.id}
                className="rounded-xl border border-slate-200/70 bg-white/80 p-3 dark:border-slate-700/70 dark:bg-slate-900/70"
              >
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">{account.accountName}</p>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <GlassCard className="space-y-3">
          <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Deal Management</p>
          <SimpleTable
            rows={deals}
            columns={[
              ...
              { key: 'salesRepresentative', label: 'Sales Rep' },
              {
                key: 'value',
                label: 'Value',
                render: (deal) => <span className="font-semibold text-slate-700 dark:text-slate-200">{formatCurrency(deal.value)}</span>,
              },
              {
                key: 'stage',
                label: 'Stage',
      ...
      <section className="grid gap-4 xl:grid-cols-[1fr_1fr]">
        <GlassCard className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Sales Target Management</p>
        ...
        <GlassCard className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Sales Team Performance</p>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        <GlassCard>
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Customer Interaction History</p>
          </div>
        ...
        <GlassCard>
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Deal Tracking Automation</p>
          </div>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
        ...
        <GlassCard>
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Commission Automation</p>
          </div>
