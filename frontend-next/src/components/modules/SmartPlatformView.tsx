...
export function SmartPlatformView() {
 ...
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 ">Stability Focus</p>
 <div className="mt-3 space-y-2">
 {data.stabilityFocus.map((item) => (
 <div key={item} className="rounded-xl border border-slate-200/70 bg-white/70 p-3 text-sm text-slate-600 ">
 {item}
 </div>
 ))}
 </div>
 ...
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 ">Launch Checklist</p>
 <div className="mt-3 space-y-2">
 {data.launchChecklist.map((item) => (
 <div key={item.item} className="flex items-start gap-2 rounded-xl bg-emerald-50 p-3 text-sm text-emerald-800 ">
