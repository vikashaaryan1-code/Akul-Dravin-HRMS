...
const DEMO_CYCLES: Cycle[] = []; // No cycles in healthy state

const DEMO_RESOURCE_OWN: ResOwn[] = [
  {
...
const NODE_STYLE: Record<NodeType, { color: string; bg: string; border: string; icon: React.ElementType }> = {
  ...
  RESOURCE:       { color: 'text-slate-400',   bg: 'bg-slate-800/60',   border: 'border-slate-700/40',   icon: Target },
};
...
function ResourceOwnershipPanel({ ownership }: { ownership: ResOwn[] }) {
  return (
    <div className="space-y-2">
      ...
      {ownership.map(res => {
        ...
        return (
          <div key={res.resource} className={`rounded-xl border p-3 ${hot ? 'border-amber-500/30 bg-amber-500/5' : 'border-slate-700/30 bg-slate-800/40'}`}>
            <div className="flex items-center gap-2 mb-2">
              <Target className={`h-3.5 w-3.5 ${hot ? 'text-amber-400' : 'text-slate-500'}`} />
              <span className="font-mono text-xs font-bold text-slate-300">{res.resource}</span>
...
function WaitChainPanel({ chains, nodeMap }: { chains: ChainEntry[]; nodeMap: Map<string, WFNode> }) {
  ...
  if (chains.length === 0) return (
    <div className="flex items-center gap-2 p-3 rounded-xl bg-slate-800/30 border border-slate-700/20 text-[10px] text-slate-600">
    ...
    <div className="space-y-2">
      ...
      <div className="space-y-1">
        {edges.map((edge, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-2 py-1 border-b border-slate-800/60 last:border-0">
              {fromNode ? <NodeChip node={fromNode} /> : <span className="text-[9px] text-slate-700">{edge.from}</span>}
              <div className="flex items-center gap-1 flex-shrink-0">
                <div className="w-4 h-px bg-slate-700" />
                <span className={`text-[8px] font-black ${es.color} px-1 py-0.5 rounded bg-slate-900/50`}>{es.label}</span>
                <div className="w-4 h-px bg-slate-700" />
              </div>
...
function ConfidencePanel({ records }: { records: ConfRecord[] }) {
  return (
    <div className="space-y-2">
      ...
      {records.map(rec => (
        <div key={rec.planId} className={`rounded-xl border p-3 ${rec.recommended ? 'border-emerald-500/25 bg-emerald-500/5' : 'border-slate-700/25 bg-slate-800/30'}`}>
          <div className="flex items-start justify-between gap-2 mb-2">
            <div>
              <div className="flex items-center gap-1.5">
                <span className="text-xs font-bold text-slate-200">{rec.planName}</span>
          ...
          <div className="grid grid-cols-3 gap-2 text-[10px]">
            ...
            <div>
              ...
              <div className="flex items-center gap-1">
                ...
                <span className={`font-bold ${rec.compensationRate < 30 ? 'text-slate-400' : 'text-orange-400'}`}>{rec.compensationRate}%</span>
              </div>
            </div>
            <div>
              <p className="text-slate-600 mb-0.5">Median Duration</p>
              <span className="font-bold text-slate-300">{rec.medianDurationMin !== null ? `${rec.medianDurationMin}m` : '—'}</span>
            </div>
          </div>
        </div>
      ))}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-5 gap-2">
        {[
          { label: 'Nodes',    value: stats.nodes,    icon: Target,      color: 'text-slate-400' },
          { label: 'Edges',    value: stats.edges,    icon: Link,        color: 'text-slate-400' },
          ...
          const I = s.icon;
          return (
            <div key={s.label} className="p-2.5 rounded-xl bg-slate-800/50 border border-slate-700/30 text-center">
      ...
      <div className="flex gap-1 bg-slate-800/50 rounded-xl p-1 border border-slate-700/30">
        {(['ownership', 'chains', 'edges', 'confidence'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`flex-1 py-1.5 rounded-lg text-[10px] font-semibold transition capitalize ${tab === t ? 'bg-slate-700/70 text-slate-200' : 'text-slate-500 hover:text-slate-400'}`}>
