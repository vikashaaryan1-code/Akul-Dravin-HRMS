...
export function WhiteLabelModuleView() {
 ...
 className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition ${activeTab === tab.id ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700 '}`}>
 {tab.icon}{tab.label}
 </button>
 ))}
 </div>
 ...
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-4">Brand Identity</p>
 ...
 {[...Array(4)].map((_,i) => <div key={i} className="h-2.5 rounded bg-slate-100 " style={{width:`${70-i*10}%`}} />)}
 </div>
 </div>
 </div>
 ...
 <p className="mt-2 text-xs text-slate-500 text-center italic">"{config.loginTagline}"</p>
 </GlassCard>
 </div>
 )}
 ...
 {activeTab === 'domain' && (
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-4">Custom Domain Configuration</p>
 ...
 {activeTab === 'email' && (
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-4">Email / SMTP Configuration</p>
 ...
 {activeTab === 'features' && (
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-4">Feature Toggles</p>
 <div className="space-y-3">
 {FEATURE_TOGGLES.map((feature) => (
 <div key={feature.key} className="flex items-center justify-between p-4 rounded-xl border border-slate-100 hover:bg-slate-50 transition">
 <div>
 ...
 className={`relative h-6 w-11 rounded-full transition-colors ${toggles[feature.key] ? 'bg-blue-600' : 'bg-slate-200 '}`}
 >
