...
export function OffboardingModuleView() {
  ...
  // Simulate: last 3 employees in offboarding
  const offboardList: OffboardRow[] = employees.slice(-3).map((e, i) => ({
    id: e.id,
    name: e.name,
  ...
  return (
    <div className="space-y-5 animate-rise">
      ...
      />
      <GlassCard>
        <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 mb-3">Offboarding Checklist</p>
        <div className="flex flex-col gap-2">
          {OFFBOARDING_STEPS.map((step, i) => (
            <div key={step} className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
              <span className="h-6 w-6 rounded-full bg-red-50 dark:bg-red-900/20 text-red-500 flex items-center justify-center text-xs font-bold">{i + 1}</span>
