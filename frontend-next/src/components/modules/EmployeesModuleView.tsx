...
import { AKUL_DRAVIN_A2Z_ACTIVE_MODULES, AKUL_DRAVIN_A2Z_SERVICE_SUITES } from '@/lib/public-site';
...
export function EmployeesModuleView() {
 ...
 <p className="text-sm font-semibold text-slate-800 ">Workforce posture stable</p>
 </div>
 ...
 className="rounded-full border border-slate-300 bg-white px-4 py-2 text-xs font-semibold text-slate-700 transition hover:border-slate-400 disabled:cursor-not-allowed disabled:opacity-45 "
 >
 Export Roster
 </button>
 <button
 ...
 className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-semibold text-white shadow-md transition hover:bg-slate-50 "
 >
 Add Employee
 </button>
 {canOpenDocuments ? (
 <Link href={`/documents?role=${safeRole}`} className="rounded-full border border-slate-300 bg-white px-4 py-2 text-xs font-semibold text-slate-700 transition hover:border-slate-400 ">
 Document Center
 </Link>
 ) : null}
 </div>
 ...
 return (
 <div key={item.label} className="rounded-3xl border border-white/70 bg-white/72 p-4 shadow-sm backdrop-blur ">
 <div className="flex items-start justify-between gap-3">
 <div>
 <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 ">{item.label}</p>
 <p className="mt-3 text-2xl font-semibold text-slate-900 ">{item.value}</p>
 </div>
 <div className="rounded-2xl border border-white/70 bg-white/70 p-2.5 text-aqua ">
 <Icon size={18} />
 </div>
 </div>
 ...
 <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
 <div>
 <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 ">AKUL DRAVIN A2Z sync</p>
 <h2 className="mt-2 text-xl font-semibold text-slate-900 ">Atlas modules are now mapped into the HRMS workspace</h2>
 <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600 ">
 {AKUL_DRAVIN_A2Z_ACTIVE_MODULES} AKUL DRAVIN modules across {AKUL_DRAVIN_A2Z_SERVICE_SUITES.length} grouped service suites are now accessible from AKUL DRAVIN HRMS, not only the public A2Z page.
 </p>
 </div>
 <Link
 ...
 {AKUL_DRAVIN_A2Z_SERVICE_SUITES.map((suite) => (
 <span
 key={suite.id}
 className="rounded-full border border-slate-300/80 bg-white/80 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-600 "
 >
 {suite.title}
 </span>
 ))}
 ...
 <p className="mt-5 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-600">Roster spotlight</p>
 <h2 className="mt-2 text-2xl font-semibold">{spotlight.name}</h2>
 <p className="mt-2 text-sm leading-6 text-slate-600">{spotlight.designation} in {spotlight.department}. Manager: {spotlight.manager}.</p>
 <div className="mt-4 flex flex-wrap gap-2">
 ...
 <p className="text-[11px] uppercase tracking-[0.12em] text-slate-600">Performance</p>
 <p className="mt-2 text-2xl font-semibold">{spotlight.score}%</p>
 <p className="mt-2 text-xs text-slate-500">{spotlight.location}</p>
 </div>
 <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
 <p className="text-[11px] uppercase tracking-[0.12em] text-slate-600">Automation</p>
 <p className="mt-2 text-2xl font-semibold">{spotlight.automation}%</p>
 <p className="mt-2 text-xs text-slate-500">{spotlight.email}</p>
 </div>
 </div>
 ...
 <div>
 <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 ">Search & filters</p>
 <h2 className="mt-2 text-xl font-semibold text-slate-900 ">Workforce explorer</h2>
 </div>
 <div className="text-xs text-slate-500 ">{filteredRows.length} results in current view</div>
 </div>
 ...
 <input value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent text-sm outline-none placeholder:text-slate-500" placeholder="Search name, employee id, department or manager" />
 </label>
 <select value={departmentFilter} onChange={(event) => setDepartmentFilter(event.target.value)} className="rounded-2xl border border-slate-200 bg-white px-3 py-3 text-sm outline-none transition focus:border-aqua ">
 {departments.map((department) => (
 <option key={department} value={department}>{department}</option>
 ))}
 </select>
 <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as 'All' | EmployeeRecord['status'])} className="rounded-2xl border border-slate-200 bg-white px-3 py-3 text-sm outline-none transition focus:border-aqua ">
 ...
 <button key={employee.id} type="button" onClick={() => setSelectedId(employee.id)} className={`rounded-[26px] border p-4 text-left shadow-panel transition hover:-translate-y-0.5 ${employee.id === spotlight.id ? 'border-aqua/40 bg-slate-50 text-white' : 'border-slate-200/80 bg-white/90 '}`}>
 <div className="flex items-center justify-between gap-3">
 <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-ember to-amber text-sm font-bold text-navy">{employee.avatar}</span>
 <StatusPill label={employee.status} tone={employee.status === 'Active' ? 'success' : employee.status === 'On Leave' ? 'warning' : 'default'} />
 </div>
 <p className={`mt-4 text-[11px] font-semibold uppercase tracking-[0.12em] ${employee.id === spotlight.id ? 'text-slate-600' : 'text-slate-500 '}`}>{employee.department}</p>
 <h3 className="mt-2 text-lg font-semibold">{employee.name}</h3>
 <p className={`text-sm ${employee.id === spotlight.id ? 'text-slate-600' : 'text-slate-600 '}`}>{employee.designation}</p>
 ...
 <div className="flex items-center justify-between gap-3">
 <div>
 <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 ">Employee matrix</p>
 <h2 className="mt-2 text-xl font-semibold text-slate-900 ">Detailed roster table</h2>
 </div>
 ...
 <span className="text-xs text-slate-500 ">{employee.manager}</span>
 </button>
 ),
 },
 ...
 <div className="flex items-center justify-between gap-3">
 <div>
 <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 ">Pinned roster</p>
 <h2 className="mt-2 text-xl font-semibold text-slate-900 ">Command rail</h2>
 </div>
 <Star size={18} className="text-amber-500" />
 </div>
 <div className="mt-5 space-y-3">
 {pinnedRows.map((employee) => (
 <button key={employee.id} type="button" onClick={() => setSelectedId(employee.id)} className="flex w-full items-center justify-between gap-3 rounded-2xl border border-slate-200/80 bg-white/80 p-3 text-left transition hover:border-aqua/40 ">
 <div>
 <p className="text-sm font-semibold text-slate-800 ">{employee.name}</p>
 <p className="text-xs text-slate-500 ">{employee.department} - {employee.designation}</p>
 </div>
 <p className="text-sm font-semibold text-slate-900 ">{employee.score}%</p>
 </button>
 ))}
 </div>
 </GlassCard>
 ...
 <GlassCard>
 <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 ">Signal stack</p>
 <h2 className="mt-2 text-xl font-semibold text-slate-900 ">People ops notes</h2>
 <div className="mt-5 space-y-3">
 {[
 ...
 <div>
 <p className="text-sm font-semibold text-slate-800 ">Document readiness</p>
 <p className="text-xs text-slate-500 ">Linked employee document visibility</p>
 </div>
 </div>
 <p className="mt-4 text-sm text-slate-600 ">Document Center now feels connected to the employee roster instead of living as an isolated block.</p>
 </GlassCard>
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 ">Attendance pulse</p>
 <p className="mt-2 text-sm text-slate-600 ">Attendance, tracking, and role context are now tied directly into the same employee command flow.</p>
 </GlassCard>
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 ">Performance context</p>
 <p className="mt-2 text-sm text-slate-600 ">Performance score and automation readiness are visible together so the page feels more premium and actionable.</p>
 </GlassCard>
 </section>
