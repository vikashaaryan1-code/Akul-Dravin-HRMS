...
/* ── Event Styling ───────────────────────────────────────────────────────────── */ const EVENT_META: Record<IncidentEventType, { icon: any; color: string; bgColor: string; label: string }> = {
 ...
 DOMAIN_MUTATION: { icon: GitBranch, color: 'text-slate-500', bgColor: 'bg-slate-700/30 border-slate-600/30', label: 'Domain Mutation' },
};
...
/* ── Component ───────────────────────────────────────────────────────────────── */ export function IncidentReplayView() {
 ...
 <span className="flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-700/60 border border-slate-600/40 text-slate-500 text-[10px]">{timeline.events.length} events total</span>
 </div>
 </div>
 ...
 className={`ml-auto flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition ${showCausal ? 'bg-indigo-600 text-white' : 'bg-slate-50/70 text-slate-500 hover:text-slate-700'}`}>
 ...
 <div className="p-3 rounded-2xl bg-slate-50/60 border border-slate-200/50 space-y-2">
 <p className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Selected Event</p>
 <p className="text-xs font-bold text-slate-700">{EVENT_META[selectedEvent.type].label}</p>
 <p className="text-[10px] text-slate-500 leading-relaxed">{selectedEvent.description}</p>
 ...
 {causalOf.length > 0 && (
 <div className="p-3 rounded-2xl bg-slate-50/40 border border-slate-200/30 space-y-1.5">
 ...
 <span className="text-[10px] text-slate-500 truncate">{e.description.slice(0, 60)}…</span>
 </div>
 );
 })}
 </div>
 ...
 {selectedEvent.sloId && (
 <div className="p-2 rounded-xl bg-slate-50/30 border border-slate-200/30">
 ...
 {/* Layer legend */}
 <div className="p-3 rounded-xl bg-slate-50/30 border border-slate-200/30 grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1">
