...
const ENTITY_COLORS: Record<string, string> = {
  employee: 'bg-blue-100 dark:bg-blue-900/30 text-blue-600',
  crm_lead: 'bg-violet-100 dark:bg-violet-900/30 text-violet-600',
  payroll: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600',
  recruitment: 'bg-amber-100 dark:bg-amber-900/30 text-amber-600',
  leave: 'bg-cyan-100 dark:bg-cyan-900/30 text-cyan-600',
  compliance: 'bg-rose-100 dark:bg-rose-900/30 text-rose-600',
  workflow: 'bg-slate-100 dark:bg-slate-800 text-slate-600',
  ai_insight: 'bg-purple-100 dark:bg-purple-900/30 text-purple-600',
};
  ...
  return (
    <div className="space-y-5 animate-rise">
      <div className="flex items-start justify-between">
        ...
        <button onClick={refresh} disabled={loading} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition disabled:opacity-50">
      ...
      <div className="flex gap-1.5 flex-wrap">
        {FILTER_TYPES.map(type => (
          <button key={type} onClick={() => setFilter(type)}
            className={`px-3 py-1 rounded-full text-xs font-medium capitalize transition ${filter === type ? 'bg-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
      ...
      <GlassCard>
        <div className="relative">
          ...
          <div className="absolute left-[27px] top-4 bottom-4 w-px bg-slate-100 dark:bg-slate-800" />

          <div className="space-y-0">
            {filtered.map((event, i) => (
              <div key={event.id} className={`flex items-start gap-3 py-3.5 ${i < filtered.length - 1 ? 'border-b border-slate-50 dark:border-slate-800/60' : ''}`}>
                {/* Entity type icon */}
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="text-[10px] text-slate-400 whitespace-nowrap shrink-0">{timeAgo(event.createdAt)}</span>
                  </div>
                </div>
              </div>
            ))}
            ...
            {filtered.length === 0 && (
              <div className="py-10 text-center text-sm text-slate-400">No activity events for this filter.</div>
            )}
          </div>
        </div>
      </GlassCard>
