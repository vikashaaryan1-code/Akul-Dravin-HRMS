...
/* ── Constants ───────────────────────────────────────────────────────────────── */ const TRIGGERS: { value: TriggerType; label: string; icon: React.ReactNode; color: string }[] = [
...
const STEP_TYPES: { type: StepType; label: string; icon: React.ReactNode; color: string; defaultName: string }[] = [
 ...
 { type: 'ai_action', label: 'AI Action', icon: <Zap className="h-4 w-4" />, color: 'text-purple-600 bg-purple-50 ', defaultName: 'Generate AI Insight' },
];
...
/* ── Step Config Forms ───────────────────────────────────────────────────────── */ function StepConfigForm({ step, onChange }: { step: WorkflowStep; onChange: (cfg: Record<string, string>) => void }) {
 ...
 className="w-full px-3 py-1.5 text-xs rounded-lg border border-slate-200 bg-white text-slate-800 focus:outline-none focus:ring-1 focus:ring-blue-400" />
 </div>
 );
...
/* ── Step Card ───────────────────────────────────────────────────────────────── */ function StepCard({ step, index, onUpdate, onDelete, isLast }: {
 ...
 <div className={`rounded-xl border ${expanded ? 'border-blue-200 ' : 'border-slate-200 '} bg-white transition`}>
 ...
 <p className="text-[10px] text-slate-500">{meta?.label} · Step {index + 1}</p>
 </div>
 <div className="flex items-center gap-1">
 <button onClick={e => { e.stopPropagation(); onDelete(); }}
 className="p-1 rounded text-slate-600 hover:text-red-400 transition"><Trash2 className="h-3.5 w-3.5" /></button>
...
/* ── Main Component ──────────────────────────────────────────────────────────── */ const EMPTY_WORKFLOW: WorkflowDef = {
 name: 'New Workflow',
 description: '',
 triggerType: 'manual',
 steps: [],
...
export function WorkflowBuilderView() {
 ...
 className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 transition">
 <RotateCcw className="h-3.5 w-3.5" />Templates
 </button>
 <button onClick={testRun} disabled={testing}
 className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-xl border border-blue-200 text-blue-600 hover:bg-blue-50 transition disabled:opacity-50">
 ...
 {/* Test result */}
 {testResult && (
 <div className="px-4 py-3 rounded-xl border border-blue-200 bg-blue-50 text-sm text-blue-700 flex items-center gap-2">
 ...
 className="text-left p-4 rounded-2xl border border-slate-200 bg-white hover:border-blue-300 hover:bg-blue-50/40 transition group">
 ...
 className="w-full px-3 py-2 text-sm rounded-xl border border-slate-200 bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-400/30" />
 </div>
 <div>
 ...
 className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-400/30 resize-none" />
 </div>
 <div className="flex items-center gap-2">
 <input type="checkbox" id="wf-active" checked={workflow.isActive} onChange={e => setWorkflow(wf => ({ ...wf, isActive: e.target.checked }))}
 className="rounded" />
 <label htmlFor="wf-active" className="text-xs text-slate-600 ">Active (will execute on trigger)</label>
 </div>
 </div>
 </GlassCard>
 ...
 <p className="text-xs text-slate-500 mt-1">Add steps from the left panel or load a template</p>
 </div>
 ) : (
 ...
 <div className="flex justify-center mb-1"><ArrowDown className="h-4 w-4 text-slate-600" /></div>
 ...
 {/* End node */}
 <div className="flex justify-center mt-2"><ArrowDown className="h-4 w-4 text-slate-600" /></div>
 <div className="flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-emerald-50 text-emerald-600 text-xs font-bold mt-1">
