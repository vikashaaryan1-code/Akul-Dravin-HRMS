...
const rowColors: Record<RowStatus, string> = {
  ...
  invalid:   'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800',
  duplicate: 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800',
};
        ...
        <GlassCard>
          <div
            className={`rounded-3xl border-2 border-dashed p-12 text-center transition cursor-pointer
              ${dragging ? 'border-blue-400 bg-blue-50/30 dark:bg-blue-900/20' : 'border-slate-300 dark:border-slate-700 hover:border-blue-400'}`}
          ...
          >
            ...
            <p className="mt-2 text-sm text-slate-400">Accepts .csv files up to 10 MB</p>
            <input
              ref={fileInputRef}
              type="file"
              accept=".csv"
        ...
        <>
          ...
          <GlassCard>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { label: 'Total Rows',   value: preview.totalRows,     color: 'text-slate-700 dark:text-slate-200' },
                { label: '✅ Valid',      value: preview.validRows,     color: 'text-emerald-600 dark:text-emerald-400' },
                { label: '❌ Invalid',    value: preview.invalidRows,   color: 'text-red-600 dark:text-red-400' },
                { label: '⚠️ Duplicate', value: preview.duplicateRows, color: 'text-amber-600 dark:text-amber-400' },
              ].map(s => (
          ...
          {/* Row Details */}
          <GlassCard>
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-3">Row-by-Row Validation</p>
      ...
      {step === 'done' && (
        <GlassCard>
          <div className="text-center py-8 space-y-4">
            ...
            <h2 className="text-2xl font-semibold text-slate-800 dark:text-slate-100">Import Queued!</h2>
            {jobId && (
              <p className="text-sm text-slate-500 dark:text-slate-400">Job ID: <code className="font-mono">{jobId}</code></p>
            )}
