...
export function WhiteLabelAdminView() {
  ...
  // Branding state
  const [brandName, setBrandName]         = useState('Akul Dravin HRMS');
    ...
    <div className="space-y-5">
        ...
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold transition ${
              tab === t.id ? 'bg-indigo-600 text-white' : 'bg-slate-800/60 text-slate-400 hover:text-slate-200'
            }`}>
            {t.icon}{t.label}
          </button>
        ))}
      ...
      {tab === 'branding' && (
        <div className="grid gap-4 xl:grid-cols-[1fr_320px]">
          <GlassCard>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">Brand Identity</p>
            <div className="space-y-4">
              <div>
                <label className="text-xs text-slate-400 mb-1 block">Brand Name</label>
                <input value={brandName} onChange={e => setBrandName(e.target.value)}
                  className="w-full px-3 py-2 text-sm rounded-xl bg-slate-800 border border-slate-700 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/30" />
              </div>
              <div>
                <label className="text-xs text-slate-400 mb-1 block">Login Tagline</label>
                ...
                {[
                  ...
                  <div key={c.label}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-mono text-slate-400">{c.value}</span>
                    </div>
                  </div>
                ))}
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={logoUrl} onChange={e => setLogoUrl(e.target.value)} placeholder="https://..."
                    className="w-full px-3 py-2 text-xs rounded-xl bg-slate-800 border border-slate-700 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/30" />
                </div>
                <div>
              ...
              <div>
                <label className="text-xs text-slate-400 mb-1 block">Custom CSS</label>
            ...
            <div className="rounded-xl overflow-hidden border border-slate-700/50">
              ...
              <div className="h-32 flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${primaryColor}22, ${secondaryColor}22)` }}>
                {logoUrl
                  ...
                  : <div className="h-12 w-12 rounded-2xl flex items-center justify-center text-white font-bold text-lg" style={{ background: primaryColor }}>{brandName[0]}</div>
                }
              </div>
              <div className="p-4 bg-slate-900">
                <p className="font-bold text-slate-200 text-sm">{brandName}</p>
      ...
      {tab === 'domain' && (
        <GlassCard>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">Custom Domain Configuration</p>
          <div className="space-y-4 max-w-lg">
            <div className="flex items-center gap-2">
              <div className={`h-2 w-2 rounded-full ${domainVerified ? 'bg-emerald-400' : 'bg-amber-400'}`} />
              <span className="text-xs text-slate-400">{domainVerified ? 'Domain verified and active' : 'No verified domain'}</span>
            </div>
            <div>
              <label className="text-xs text-slate-400 mb-1 block">Custom Domain</label>
            ...
            {dnsInstructions && (
              <div className="relative p-4 rounded-xl bg-slate-900 border border-slate-700 font-mono text-xs text-slate-300 whitespace-pre-line">
                {dnsInstructions}
      ...
      {tab === 'entitlements' && (
        <div className="grid gap-4 xl:grid-cols-2">
          <GlassCard>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">Plan & Caps</p>
            <div className="space-y-4">
              <div>
                  ...
                  {(['starter','professional','enterprise','white_label'] as Plan[]).map(p => (
                    <button key={p} onClick={() => setPlan(p)}
                      className={`py-2 px-3 rounded-xl text-xs font-semibold capitalize transition border ${
                        plan === p ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-slate-800 text-slate-400 border-slate-700 hover:text-slate-200'
                      }`}>{p.replace('_',' ')}</button>
                  ))}
                </div>
              </div>
              {[
                ...
                <div key={row.label} className="flex justify-between items-center py-2 border-b border-slate-700/50">
          ...
          <GlassCard>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">Module Access</p>
            <div className="grid grid-cols-2 gap-2">
              {ALL_MODULES.map(m => {
                ...
                return (
                  <button key={m} onClick={() => toggleModule(m)}
                    className={`flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs font-medium transition border ${
                      active ? 'bg-indigo-600/20 border-indigo-500/40 text-indigo-300' : 'bg-slate-800/60 border-slate-700/50 text-slate-500 hover:text-slate-300'
                    }`}>
