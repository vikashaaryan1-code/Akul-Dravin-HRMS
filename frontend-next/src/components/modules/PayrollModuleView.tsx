...
/* ── Helper ──────────────────────────────────────────────────────────────────── */ function formatPeriod(item: PayrollItemApiRecord): string {
 if (item.metadata?.period) return item.metadata.period;
 /* Fall back to createdAt month/year */ const d = new Date(item.createdAt);
 return `${String(d.getMonth() + 1).padStart(2, '0')}/${d.getFullYear()}`;
}

/* ── Payslip Row ─────────────────────────────────────────────────────────────── */ function PayslipRow({ item }: { item: PayrollItemApiRecord }) {
 ...
 className="flex shrink-0 items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-[11px] font-bold text-slate-600 transition-all hover:border-white/20 hover:bg-white/10 hover:text-white disabled:opacity-50"
...
/* ── Run Payroll Panel ──────────────────────────────────────────────────────── */ function RunPayrollPanel() {
...
/* ── CTC & Tax Calculator ────────────────────────────────────────────────────── */ function CTCCalculatorWidget() {
 const [ctc, setCtc] = useState<number>(1200000);
 
 const basic = ctc * 0.50;
 const hra = basic * 0.40; /* Assuming non-metro */ const specialAllowance = ctc - (basic + hra);
 
 /* Simple Old Regime Tax Calc (just for demo purposes) */ let tax = 0;
 ...
 />
 <div className="text-xl font-bold text-navy text-center">
 {formatCurrency(ctc)}
 </div>
 </div>
 ...
 <p className="font-medium text-slate-600">{formatCurrency(basic)}</p>
 </div>
 <div>
 <p className="text-xs text-slate-500 mb-1">HRA (40% of Basic)</p>
 <p className="font-medium text-slate-600">{formatCurrency(hra)}</p>
 </div>
 <div>
 <p className="text-xs text-slate-500 mb-1">Special Allowance</p>
 <p className="font-medium text-slate-600">{formatCurrency(specialAllowance)}</p>
 </div>
 <div>
 ...
 <span className="text-xs font-semibold text-slate-500">Est. Monthly Take-Home</span>
...
/* ── Main Component ──────────────────────────────────────────────────────────── */ export function PayrollModuleView() {
 const activeRole = useUIStore((state) => state.activeRole);

 /* Payroll batches (existing) */ const { data, isLive, loading, error } = useApiResource({
 loader: async () => {
 ...
 /* My Payslips (self-service — independent fetch) */ const {
 data: payslipData,
 isLive: payslipsLive,
 ...
 // No early return for empty data; let it compute zeros if needed.
 ...
 <div>
 <h2 className="text-sm font-bold text-navy">My Payslips</h2>
 ...
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 ">Target-Based Salary Automation</p>
 ...
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 ">Days-Wise Payroll Calculation</p>
 <p className="mt-2 text-sm text-slate-600 ">
