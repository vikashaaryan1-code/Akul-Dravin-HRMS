...
/* --- Profile Tab --- */ function ProfileTab({ user }: { user: any }) {
 ...
 {/* Avatar */}
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-4">Profile Photo</p>
 <div className="flex items-center gap-5">
 <div className="relative group">
 <div className="h-20 w-20 rounded-2xl bg-gradient-to-br from-blue-600 to-violet-600 flex items-center justify-center text-navy text-2xl font-bold shrink-0 overflow-hidden">
 {form.avatarUrl ? (
 ...
 <button onClick={() => setForm(prev => ({...prev, avatarUrl: ''}))} className="inline-flex items-center gap-1 rounded-full border border-slate-300 px-4 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-50 transition">
 Remove
 </button>
 </div>
 </div>
 ...
 className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-500 transition"
 />
 </div>
 <div>
 ...
 className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-500 transition resize-none"
 />
 </div>
 </div>
...
/* --- Notifications Tab --- */ function NotificationsTab() {
 ...
 {cat.items.map(item => (
 <label key={item.key} className="flex items-center justify-between p-3 rounded-xl border border-slate-100 cursor-pointer hover:bg-slate-50 transition">
 <div>
 ...
 className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${prefs[item.key] ? 'bg-blue-600' : 'bg-slate-200 '}`}
 >
...
/* --- Integrations Tab --- */ function IntegrationsTab() {
 ...
 const integrations = [
 { id: 'google', name: 'Google Workspace', desc: 'Calendar, Drive, and Meet sync', icon: '🟦', color: 'bg-blue-50 ' },
 { id: 'slack', name: 'Slack', desc: 'Team notifications and alerts', icon: '🟪', color: 'bg-violet-50 ' },
 { id: 'whatsapp', name: 'WhatsApp Business', desc: 'Employee messaging and alerts', icon: '🟩', color: 'bg-emerald-50 ' },
 { id: 'zoom', name: 'Zoom', desc: 'Video interview integration', icon: '🟦', color: 'bg-blue-50 ' },
 { id: 'jira', name: 'Jira', desc: 'Project and task tracking sync', icon: '🟧', color: 'bg-orange-50 ' },
 { id: 'zapier', name: 'Zapier', desc: 'Workflow automation triggers', icon: '🟠', color: 'bg-orange-50 ' },
 ];

 return (
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-4">Connected Integrations</p>
 <div className="grid gap-3 sm:grid-cols-2">
 {integrations.map(integration => (
 <div key={integration.id} className={`p-4 rounded-xl border ${connected[integration.id] ? 'border-emerald-200 bg-emerald-50/50 ' : 'border-slate-200 bg-white '} transition`}>
 ...
 className={`mt-3 w-full rounded-lg py-1.5 text-xs font-semibold transition ${connected[integration.id] ? 'bg-red-50 text-red-600 hover:bg-red-100 ' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
 >
...
/* --- Billing Tab --- */ function BillingTab({ accessToken }: { accessToken: string | null }) {
 ...
 <div className="space-y-5">
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-4">Current Plan</p>
 <div className="flex items-center gap-4 p-4 rounded-xl bg-gradient-to-r from-blue-50 to-violet-50 border border-blue-200 ">
 ...
 <p className="text-xl font-black text-slate-900 mt-1">{plan.price}<span className="text-xs font-normal text-slate-500">/mo</span></p>
 <ul className="mt-2 space-y-1 text-xs text-slate-600 ">
 <li>👥 {plan.seats} seats</li>
 <li>🧩 {plan.modules} modules</li>
 </ul>
 {plan.active ? (
 <div className="mt-3 text-center text-xs font-semibold text-blue-600 ">Current Plan</div>
 ) : (
 <button className="mt-3 w-full rounded-lg bg-slate-100 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-200 transition">
 Switch Plan
 </button>
 )}
 </div>
 ...
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-3">Recent Invoices</p>
 ...
 <div key={inv.month} className="flex items-center justify-between p-3 rounded-xl border border-slate-100 ">
 <div>
 <p className="text-sm font-medium text-slate-800 ">{inv.month}</p>
 <p className="text-xs text-slate-500">{inv.amount}</p>
 </div>
 <div className="flex items-center gap-2">
 <span className="rounded-full bg-emerald-100 text-emerald-700 px-2.5 py-0.5 text-xs font-semibold">{inv.status}</span>
...
/* --- Main Settings Component --- */ export function SettingsModuleView() {
 ...
 {/* Tab Navigation */}
 <div className="flex gap-1 overflow-x-auto border-b border-slate-200 pb-0 scrollbar-hide">
 {TABS.map(tab => {
 const Icon = tab.icon;
 return (
 <button
 ...
 <TwoFactorSetupView />
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-4">Appearance</p>
 <div className="flex items-center justify-between p-3 rounded-xl border border-slate-100 ">
 <div>
 <p className="text-sm font-medium text-slate-800 ">Theme Mode</p>
 <p className="text-xs text-slate-500 mt-0.5">Currently: {theme === 'light' ? 'Light Mode' : 'Dark Mode'}</p>
 </div>
 <button
 onClick={toggleTheme}
 disabled={!canUpdatePreferences}
 className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50 transition"
 >
 ...
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-4">Session Management</p>
 <div className="space-y-3">
 <div className="flex items-center justify-between p-3 rounded-xl border border-slate-100 bg-emerald-50/30 ">
 <div className="flex items-center gap-3">
 ...
 className="flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-100 transition"
 >
