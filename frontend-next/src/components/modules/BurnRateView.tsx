    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Rolling reliability capital · burn velocity · depletion forecast</p>
        </div>
        {(fastBurns > 0) && (
      ...
      <div className="flex gap-2 flex-wrap">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setPanel(t.id)}
            className={`relative px-3.5 py-2 rounded-xl text-xs font-semibold transition ${panel === t.id ? 'bg-indigo-600 text-white' : 'bg-slate-800/70 text-slate-400 hover:text-slate-200'}`}>
            {t.label}
            {t.badge !== undefined && (
      ...
      {panel === 'gauges' && (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {BUDGETS.map(b => {
            ...
            return (
              <div key={b.sloId} className={`p-4 rounded-2xl border ${over ? 'bg-red-500/10 border-red-500/30' : warn ? 'bg-amber-500/10 border-amber-500/30' : 'bg-slate-800/60 border-slate-700/50'}`}>
                <p className="text-xs font-semibold text-slate-300 mb-3">{b.name}</p>
                ...
                <div className="flex justify-between text-[10px]">
                  <span className={`font-bold text-base ${over ? 'text-red-300' : warn ? 'text-amber-300' : 'text-slate-200'}`}>
      ...
      {panel === 'burnrate' && (
        <div className="space-y-2">
          ...
          {BURN_RATES.map((r, i) => (
            <div key={i} className={`grid grid-cols-7 gap-2 items-center px-3 py-2.5 rounded-xl border ${r.isFastBurn ? 'bg-red-500/10 border-red-500/25' : r.isSlowBurn ? 'bg-orange-500/10 border-orange-500/25' : 'bg-slate-800/60 border-slate-700/50'}`}>
              <span className="col-span-2 text-xs font-semibold text-slate-200 truncate">{SLO_NAMES[r.sloId]}</span>
              <span className="text-xs font-mono text-slate-400">{r.window}</span>
              <span className={`text-sm font-bold ${r.isFastBurn ? 'text-red-300' : r.isSlowBurn ? 'text-orange-300' : 'text-emerald-300'}`}>{r.rate}×</span>
              <BurnBadge r={r} />
              <TrendIcon t={r.trend} />
              <span className="text-xs text-slate-400">{fmtHours(r.forecastExhaustionMin)}</span>
            </div>
          ))}
          <div className="mt-3 p-3 rounded-xl bg-slate-800/40 border border-slate-700/30 text-[10px] text-slate-500 space-y-0.5">
        ...
        <div className="space-y-4">
          <div className="flex gap-2 flex-wrap">
            {(Object.keys(SLO_NAMES) as SloId[]).map(id => (
              <button key={id} onClick={() => setSelSlo(id)}
                className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold transition ${selSlo === id ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200'}`}>
                {SLO_NAMES[id]}
              </button>
            ))}
          </div>
          <div className="space-y-2">
            {WINDOWS.filter(w => w.sloId === selSlo).map((w, i) => (
              <div key={i} className="p-3.5 rounded-2xl bg-slate-800/60 border border-slate-700/50">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded-lg bg-slate-700 text-slate-300 text-[10px] font-bold">{w.window}</span>
                    <TrendIcon t={w.trend} />
                ...
                <div className="grid grid-cols-3 gap-3 text-[11px]">
                  ...
                  <div><p className="text-slate-500">p95 value</p><p className="text-slate-200 font-bold">{w.p95Value !== null ? (w.p95Value > 1000 ? `${Math.round(w.p95Value/1000)}s` : `${w.p95Value}s`) : '—'}</p></div>
                  <div><p className="text-slate-500">Violations</p><p className="text-slate-200 font-bold">{w.breachSamples}</p></div>
                </div>
        ...
        <div className="space-y-3">
              ...
              <div key={i} className={`p-4 rounded-2xl border ${urgent ? 'bg-red-500/10 border-red-500/30' : 'bg-amber-500/10 border-amber-500/30'}`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm font-bold text-slate-200">{SLO_NAMES[r.sloId]}</p>
          ...
          {BURN_RATES.filter(r => r.forecastExhaustionMin !== null).length === 0 && (
            <div className="py-16 flex flex-col items-center text-slate-600">
              ...
              <p className="text-slate-400 font-semibold">All budgets healthy</p>
