...
export function AutomationModuleView() {
 ...
 <div>
 <p className="text-sm font-semibold text-slate-800 ">Internship Certificate Lane</p>
 ...
 <p className="mt-2 text-sm font-semibold text-slate-900 ">{internshipWorkflow?.triggerType ?? 'manual-or-completion'}</p>
 </div>
 <div className="rounded-lg border border-slate-200/80 bg-white/70 px-4 py-3 ">
 <p className="text-xs uppercase tracking-[0.12em] text-slate-500">Success Rate</p>
 <p className="mt-2 text-sm font-semibold text-slate-900 ">{internshipWorkflow?.successRate ?? '0.00'}%</p>
 </div>
 <div className="rounded-lg border border-slate-200/80 bg-white/70 px-4 py-3 ">
 <p className="text-xs uppercase tracking-[0.12em] text-slate-500">Run Count</p>
 <p className="mt-2 text-sm font-semibold text-slate-900 ">{internshipWorkflow?.runCount ?? 0}</p>
 </div>
 <div className="rounded-lg border border-slate-200/80 bg-white/70 px-4 py-3 ">
 <p className="text-xs uppercase tracking-[0.12em] text-slate-500">Last Run</p>
 <p className="mt-2 text-sm font-semibold text-slate-900 ">{formatDate(internshipWorkflow?.lastRunAt)}</p>
 </div>
 </div>

 <div className="mt-4 rounded-lg border border-slate-200/80 bg-white/70 px-4 py-4 ">
 <p className="text-sm font-semibold text-slate-800 ">Latest Packet</p>
 <div className="mt-3 grid gap-3 sm:grid-cols-2">
 <div>
 <p className="text-xs uppercase tracking-[0.12em] text-slate-500">Intern</p>
 <p className="mt-2 text-sm font-semibold text-slate-900 ">{latestPacket?.internName ?? 'Ready for next run'}</p>
 </div>
 <div>
 <p className="text-xs uppercase tracking-[0.12em] text-slate-500">University</p>
 <p className="mt-2 text-sm font-semibold text-slate-900 ">{latestPacket?.university ?? 'Not generated yet'}</p>
 </div>
 <div>
 <p className="text-xs uppercase tracking-[0.12em] text-slate-500">Certificate No.</p>
 <p className="mt-2 text-sm font-semibold text-slate-900 ">{latestPacket?.certificateNumber ?? 'Generated during completion step'}</p>
 </div>
 <div>
 <p className="text-xs uppercase tracking-[0.12em] text-slate-500">Verification</p>
 <p className="mt-2 text-sm font-semibold text-slate-900 ">{latestPacket?.verificationCode ?? 'Assigned during sign-off'}</p>
 </div>
 </div>
 ...
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 ">Workflow Stages</p>
 ...
 <p className="text-sm text-slate-600 ">Workflow stages will appear here when the live config is loaded.</p>
 )}
 </div>
 </GlassCard>
 </section>
 ...
 <section className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
 <GlassCard>
 <p className="mb-3 text-sm font-semibold text-slate-800 ">Workflow Registry</p>
 <SimpleTable
 rows={data.workflows}
 columns={[
 ...
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 ">Alert Feed</p>
 <div className="mt-4 space-y-3">
 {data.alerts.length > 0 ? data.alerts.map((alert) => (
 <div key={alert.code} className="rounded-lg border border-slate-200/80 bg-white/70 px-4 py-3 ">
 <div className="flex flex-wrap items-center justify-between gap-2">
 <p className="text-sm font-semibold text-slate-900 ">{alert.code}</p>
 <StatusPill label={alert.severity} tone={workflowTone(alert.severity)} />
 </div>
 <p className="mt-2 text-sm text-slate-600 ">{alert.message}</p>
 </div>
 )) : (
 <p className="text-sm text-slate-600 ">Alerts will appear here when the automation service returns live telemetry.</p>
 )}
 </div>
 </GlassCard>
 </section>
