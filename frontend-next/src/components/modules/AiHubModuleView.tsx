        ...
        {[
          ...
          <GlassCard key={card.label}>
            ...
            <div className="relative flex items-start justify-between">
              <div>
                <p className="text-[10px] uppercase tracking-[0.13em] text-slate-400">{card.label}</p>
                <p className="mt-2 text-2xl font-bold" style={{ color: card.accent }}>{card.value}</p>
              </div>
              <span className="p-2 rounded-xl bg-slate-800/80">{card.icon}</span>
            </div>
          </GlassCard>
        ))}
      </section>
            ...
            <GlassCard>
              ...
              <div className="space-y-3">
                {ATTRITION_FALLBACK.map(emp => (
                  <div key={emp.employeeId} className="flex items-start gap-3 p-3 rounded-xl bg-slate-800/40 border border-slate-700/40">
                    <ScoreRing score={emp.riskScore} size={44} />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-sm font-semibold text-slate-200">{emp.employeeName}</p>
                        <RiskBadge level={emp.riskLevel} />
                      </div>
                      <div className="flex flex-wrap gap-1">
                        {emp.factors.map(f => (
                          <span key={f} className="text-[10px] px-1.5 py-0.5 bg-slate-700/60 text-slate-400 rounded-full">{f}</span>
                        ))}
                      </div>
                    </div>
                  </div>
            ...
            <GlassCard>
              ...
              <div className="space-y-3">
                {CANDIDATE_FIT_FALLBACK.map(c => (
                  <div key={c.candidateId} className="p-3 rounded-xl bg-slate-800/40 border border-slate-700/40">
                    <div className="flex items-center gap-3 mb-2">
                      <ScoreRing score={c.overallScore} size={44} />
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-semibold text-slate-200">{c.candidateName}</p>
                          ...
                          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                            ...
                            c.recommendation === 'PARTIAL_FIT'? 'bg-amber-500/20 text-amber-400'   :
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed border-t border-slate-700/50 pt-2 mt-1">
        ...
        <div className="space-y-3">
          <GlassCard>
            <div className="flex items-center gap-2 mb-4 pb-3 border-b border-slate-700/50">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-200">AI HR Assistant</p>
                <p className="text-xs text-emerald-400 flex items-center gap-1">
                  <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 inline-block animate-pulse" />
                  Online
                  {provider && <span className="ml-2 px-1.5 py-0.5 rounded text-[9px] font-mono bg-slate-700 text-slate-400">{provider}</span>}
                </p>
              </div>
            </div>
            ...
            <div className="h-64 overflow-y-auto space-y-3 pr-1 mb-3">
              ...
              {isTyping && (
                <div className="flex gap-2">
                  ...
                  <div className="bg-slate-800 rounded-2xl rounded-tl-sm"><TypingDots /></div>
                </div>
              )}
              <div ref={bottomRef} />
            </div>
            ...
            <form onSubmit={e => { e.preventDefault(); sendMessage(input); }} className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 text-xs rounded-xl border border-slate-700 bg-slate-800 text-slate-200 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 transition"
              />
            ...
            <div className="space-y-1.5">
              {QUICK_PROMPTS.map(p => (
                <button key={p} onClick={() => sendMessage(p)}
                  className="w-full text-left text-[11px] px-2.5 py-2 rounded-lg bg-slate-800/60 border border-slate-700/50 hover:border-indigo-500/40 hover:bg-indigo-900/20 text-slate-400 hover:text-slate-200 transition">
