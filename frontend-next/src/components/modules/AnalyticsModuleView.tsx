...
const WORKFORCE_FALLBACK: WorkforceKpiApiRecord = {
...
function KpiCard({
  ...
  return (
    <GlassCard>
      <p className="text-[10px] uppercase tracking-[0.14em] text-slate-400 mb-1">{label}</p>
...
function FunnelBar({ label, count, max }: { label: string; count: number; max: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="w-10 text-right text-xs font-semibold text-slate-300">{count}</span>
    </div>
  );
}
...
export function AnalyticsModuleView() {
  ...
  // ── Workforce KPIs
  const { data: workforce, isLive: wLive, loading: wLoading } = useApiResource({
    loader: () => platformApi.getWorkforceKpi(),
    fallback: WORKFORCE_FALLBACK,
  });

  // ── Recruitment KPIs
  const { data: recruitment, isLive: rLive, loading: rLoading } = useApiResource({
    loader: () => platformApi.getRecruitmentKpi(),
    fallback: RECRUITMENT_FALLBACK,
  });

  // ── Revenue KPIs
  const { data: revenue, isLive: revLive, loading: revLoading } = useApiResource({
  ...
  // ── Tenure distribution chart data
  ...
  // ── Revenue growth trend chart data
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        ...
        <GlassCard>
          <p className="text-xs uppercase tracking-[0.13em] text-slate-400 mb-4">
            Pipeline Funnel
