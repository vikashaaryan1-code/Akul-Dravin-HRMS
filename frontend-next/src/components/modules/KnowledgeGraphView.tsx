...
const NODE_STYLE: Record<GraphNodeType, { icon: React.ElementType; color: string; bg: string; border: string }> = {
  ...
  DOMAIN_EVENT: { icon: GitBranch,     color: 'text-slate-400',   bg: 'bg-slate-700/40',   border: 'border-slate-600/40' },
...
const EDGE_STYLE: Record<GraphEdgeRelation, { label: string; color: string }> = {
  ...
  ENQUEUED:    { label: 'Enqueued',    color: 'text-slate-400' },
...
type TraversalMode = 'none' | 'rootcause' | 'blastradius';
...
export function KnowledgeGraphView() {
  ...
  // BFS traversal
  const traversalNodes = useMemo((): Set<string> => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Causal relationships across incidents, mitigations, projections, queues, and domain events</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap text-[10px]">
          ...
          <span className="px-2 py-1 rounded-lg bg-slate-800/60 border border-slate-700/40 text-slate-400">
            {stats.edges} edges
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="pl-7 pr-3 py-1.5 rounded-xl bg-slate-800/70 border border-slate-700/40 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-indigo-500/50 w-44" />
        </div>
        {(['ALL', 'INCIDENT', 'MITIGATION', 'PROJECTION', 'QUEUE_JOB', 'DOMAIN_EVENT'] as const).map(t => (
          <button key={t} onClick={() => setFilterType(t)}
            className={`px-2.5 py-1.5 rounded-xl text-[10px] font-semibold capitalize transition ${filterType === t ? 'bg-indigo-600 text-white' : 'bg-slate-800/60 text-slate-400 hover:text-slate-200'}`}>
        ...
        {selectedNode && (
          <div className="w-64 flex-shrink-0 space-y-3">
            ...
            <div className={`p-3 rounded-2xl border ${NODE_STYLE[selectedNode.type].bg} ${NODE_STYLE[selectedNode.type].border} space-y-2`}>
              ...
              <p className="text-xs text-slate-200 font-semibold leading-snug">{selectedNode.label}</p>
            ...
            <div className="flex gap-2">
              <button onClick={() => setTraversal(traversal === 'rootcause' ? 'none' : 'rootcause')}
                className={`flex-1 flex items-center justify-center gap-1 px-2 py-1.5 rounded-xl text-[10px] font-semibold border transition ${traversal === 'rootcause' ? 'bg-red-600/30 border-red-500/40 text-red-300' : 'bg-slate-800/60 border-slate-700/40 text-slate-400'}`}>
                <ChevronLeft className="h-3 w-3" /> Root Causes
              </button>
              <button onClick={() => setTraversal(traversal === 'blastradius' ? 'none' : 'blastradius')}
                className={`flex-1 flex items-center justify-center gap-1 px-2 py-1.5 rounded-xl text-[10px] font-semibold border transition ${traversal === 'blastradius' ? 'bg-orange-600/30 border-orange-500/40 text-orange-300' : 'bg-slate-800/60 border-slate-700/40 text-slate-400'}`}>
                  ...
                  return (
                    <div key={e.to} className="flex items-start gap-1.5 cursor-pointer" onClick={ev => { ev.stopPropagation(); setSelected(e.to); }}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-slate-400 truncate">{target.label.slice(0, 55)}</p>
                ...
                {selectedEdgesIn.map(e => {
                  ...
                  return (
                    <div key={e.from} className="flex items-start gap-1.5 cursor-pointer" onClick={ev => { ev.stopPropagation(); setSelected(e.from); }}>
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={`text-[9px] font-bold ${es.color}`}>{es.label}</span>
                        <p className="text-[10px] text-slate-400 truncate">{source.label.slice(0, 55)}</p>
                      </div>
                    </div>
                  );
                })}
      ...
      {/* Legend */}
      <div className="p-3 rounded-xl bg-slate-800/30 border border-slate-700/30 grid grid-cols-3 sm:grid-cols-6 gap-x-3 gap-y-1.5">
