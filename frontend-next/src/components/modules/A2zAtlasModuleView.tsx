...
const stepToneMap: Record<(typeof AKUL_DRAVIN_A2Z_WORKFLOW_STEPS)[number]['status'], string> = {
 ...
 queued: 'bg-slate-50mber-100 text-amber-700 ',
};
...
export function A2zAtlasModuleView() {
 ...
 <span className="inline-flex rounded-full border border-slate-300/80 bg-white/80 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-600 ">
 ...
 <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600 ">{AKUL_DRAVIN_A2Z_HERO.subtitle}</p>
 <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-500 ">{AKUL_DRAVIN_A2Z_HERO.description}</p>
 </div>
 ...
 <span className="rounded-full bg-white/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-700">
 Atlas blueprint
 </span>
 </div>
 <p className="mt-5 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-600">Atlas coverage</p>
 ...
 <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-600">{suite.title}</p>
 <p className="mt-2 text-sm text-slate-500">{suite.modules.length} mapped modules</p>
 </div>
 ))}
 </div>
 <div className="mt-5 rounded-2xl border border-white/10 bg-white/5 p-4">
 <p className="text-[11px] uppercase tracking-[0.12em] text-slate-600">Primary promise</p>
 <p className="mt-2 text-sm leading-6 text-slate-700">{AKUL_DRAVIN_A2Z_HERO.title}</p>
 </div>
 </div>
 </div>
 </section>
 ...
 <div>
 <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 ">Service suites</p>
 <h2 className="mt-1 text-xl font-semibold text-slate-900 ">AKUL DRAVIN module bundles now mapped into AKUL DRAVIN</h2>
 </div>
 </div>
 ...
 >
 <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 ">Suite {index + 1}</p>
 <h3 className="mt-2 text-lg font-semibold text-slate-900 ">{suite.title}</h3>
 <p className="mt-2 text-sm leading-6 text-slate-600 ">{suite.description}</p>
 <p className="mt-3 rounded-2xl border border-white/70 bg-white/70 p-3 text-sm leading-6 text-slate-600 ">
 ...
 <GlassCard>
 <div className="flex items-center gap-3">
 <div className="rounded-2xl bg-slate-50mber/10 p-3 text-amber">
 <Workflow size={18} />
 </div>
 <div>
 <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 ">Workflow mesh</p>
 <h2 className="mt-1 text-xl font-semibold text-slate-900 ">Discovery, blueprint, and rollout are now clearly staged</h2>
 </div>
 </div>
 <div className="mt-5 grid gap-3 lg:grid-cols-3">
 {AKUL_DRAVIN_A2Z_WORKFLOW_STEPS.map((step) => (
 <article key={step.id} className="rounded-[24px] border border-slate-200/80 bg-white/80 p-4 ">
 <div className="flex items-start justify-between gap-3">
 <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 ">{step.owner}</p>
 ...
 <p className="mt-2 text-xs text-slate-500 ">{step.sla}</p>
 <p className="mt-3 text-sm leading-6 text-slate-600 ">{step.description}</p>
 <div className="mt-4 space-y-2">
 {step.outputs.map((output) => (
 <div key={output} className="flex gap-2 text-sm text-slate-600 ">
 ...
 <GlassCard>
 <div className="flex items-center gap-3">
 <div className="rounded-2xl bg-white/5 p-3 text-slate-700 ">
 <ShieldCheck size={18} />
 </div>
 <div>
 <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 ">Assurances</p>
 <h2 className="mt-1 text-xl font-semibold text-slate-900 ">What changed in the HRMS workspace</h2>
 </div>
 </div>
 <div className="mt-5 space-y-3">
 {AKUL_DRAVIN_A2Z_ASSURANCES.map((item) => (
 <article key={item.title} className="rounded-2xl border border-slate-200/80 bg-white/80 p-4 ">
 ...
 <div>
 <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 ">Next moves</p>
 <h2 className="mt-1 text-xl font-semibold text-slate-900 ">Where to use the atlas next</h2>
 </div>
 </div>
 ...
 <Link href={`/employees?role=${safeRole}`} className="rounded-full border border-slate-300 bg-white px-4 py-2 text-xs font-semibold text-slate-700 transition hover:border-slate-400 ">
 Open employees
 </Link>
 <Link href={`/dashboard?role=${safeRole}`} className="rounded-full border border-slate-300 bg-white px-4 py-2 text-xs font-semibold text-slate-700 transition hover:border-slate-400 ">
 Open dashboard
 </Link>
 </div>
 </GlassCard>
