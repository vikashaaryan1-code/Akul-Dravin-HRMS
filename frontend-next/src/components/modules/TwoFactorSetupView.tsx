...
export function TwoFactorSetupView() {
 ...
 <div>
 <p className={`text-sm font-semibold ${is2FAEnabled ? 'text-emerald-800 ' : 'text-amber-800 '}`}>
 ...
 <p className="text-base font-semibold text-slate-800 ">Set Up Authenticator App</p>
 ...
 <span className="h-5 w-5 rounded-full bg-blue-100 text-blue-600 text-[10px] font-bold flex items-center justify-center">{i + 1}</span>
 {step}
 </div>
 ))}
 </div>
 ...
 <>
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-4">Step 1 — Scan QR Code</p>
 <div className="flex flex-col sm:flex-row gap-6 items-start">
 {/* QR Code */}
 <div className="flex-shrink-0">
 {setup.qrCode ? (
 <img src={setup.qrCode} alt="TOTP QR Code" className="h-40 w-40 rounded-xl border border-slate-200 bg-white p-2" />
 ) : (
 <div className="h-40 w-40 rounded-xl border border-dashed border-slate-300 flex items-center justify-center text-slate-500 text-xs text-center p-3">
 ...
 <code className="flex-1 px-3 py-2 text-xs font-mono bg-slate-100 text-slate-700 rounded-lg border border-slate-200 tracking-widest overflow-hidden">
 {showKey ? setup.manualKey : '•'.repeat(setup.manualKey?.length ?? 32)}
 </code>
 <button onClick={() => setShowKey(s => !s)} className="p-2 rounded-lg text-slate-500 hover:text-slate-600 transition">
 ...
 <GlassCard>
 <p className="text-sm font-semibold text-slate-800 mb-3">Step 2 — Verify Code</p>
 ...
 maxLength={6}
 placeholder="000000"
 className="flex-1 px-4 py-3 text-2xl font-mono text-center tracking-[0.5em] rounded-xl border border-slate-200 bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-400/30"
 />
 ...
 <p className="text-sm font-semibold text-slate-800 ">Backup Recovery Codes</p>
 ...
 <code key={i} className="px-3 py-1.5 text-xs font-mono bg-slate-100 text-slate-700 rounded-lg border border-slate-200 text-center tracking-widest">{c}</code>
 ))}
 </div>
 </GlassCard>
 )}
 ...
 <KeyRound className="h-4 w-4 text-slate-500" /> Disable 2FA
 </p>
 ...
 className="px-4 py-2 rounded-xl border border-red-200 text-red-600 text-xs font-medium hover:bg-red-50 disabled:opacity-50 transition">
