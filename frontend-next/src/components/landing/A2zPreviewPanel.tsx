...
export function PreviewPanel({ preview, loading }: PreviewPanelProps) {
 if (loading) {
 return (
 <div className="flex animate-pulse items-center justify-center rounded-[2rem] border border-navy/10 bg-navy/5 p-20">
 ...
 if (!preview) {
 return (
 <div className="flex h-full items-center justify-center rounded-[2rem] border border-navy/5 bg-black/20 p-10 text-center">
 ...
 return (
 <div className="space-y-6 rounded-[2rem] border border-navy/10 bg-navy/5 p-8 shadow-2xl backdrop-blur-xl">
 <div className="grid grid-cols-2 gap-4">
 <div className="rounded-2xl bg-navy/5 p-4">
 <Layers className="h-5 w-5 text-amber" />
 <p className="mt-3 text-2xl font-bold text-navy">{preview.estimatedModules}</p>
 <p className="text-[10px] uppercase tracking-widest text-slate-500">Total Modules</p>
 </div>
 <div className="rounded-2xl bg-navy/5 p-4">
 <Clock className="h-5 w-5 text-aqua" />
 <p className="mt-3 text-2xl font-bold text-navy">{preview.targetTimeline}</p>
 ...
 <div key={idx} className="flex items-center justify-between rounded-xl border border-navy/5 bg-black/20 p-4">
 <div className="flex items-center gap-3">
 <CheckCircle2 className={`h-4 w-4 ${phase.status === 'ready' ? 'text-emerald-400' : 'text-slate-600'}`} />
 <span className="text-sm font-medium text-navy">{phase.phase}</span>
 </div>
