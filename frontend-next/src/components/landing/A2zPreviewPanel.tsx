...
export function PreviewPanel({ preview, loading }: PreviewPanelProps) {
  if (loading) {
    return (
      <div className="flex animate-pulse items-center justify-center rounded-[2rem] border border-white/10 bg-white/5 p-20">
        <div className="text-sm font-medium text-slate-400">Calculating your Atlas blueprint...</div>
      </div>
    );
  }
  ...
  if (!preview) {
    return (
      <div className="flex h-full items-center justify-center rounded-[2rem] border border-white/5 bg-black/20 p-10 text-center">
  ...
  return (
    <div className="space-y-6 rounded-[2rem] border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl">
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-2xl bg-white/5 p-4">
          ...
          <p className="text-[10px] uppercase tracking-widest text-slate-400">Total Modules</p>
        </div>
        <div className="rounded-2xl bg-white/5 p-4">
          <Clock className="h-5 w-5 text-aqua" />
          <p className="mt-3 text-2xl font-bold text-white">{preview.targetTimeline}</p>
          <p className="text-[10px] uppercase tracking-widest text-slate-400">Est. Timeline</p>
        </div>
      </div>

      <div className="space-y-4">
        <p className="text-[10px] uppercase tracking-widest text-slate-400">Phase Roadmap</p>
        {preview.phases.map((phase, idx) => (
          <div key={idx} className="flex items-center justify-between rounded-xl border border-white/5 bg-black/20 p-4">
