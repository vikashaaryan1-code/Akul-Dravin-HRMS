...
export function AiCopilotSection() {
 ...
 return (
 <section id="ai-copilot" className="py-28 bg-[#051124]/60" aria-labelledby="ai-heading">
 ...
 Your Enterprise
 <br />
 <span className="bg-gradient-to-r from-gold via-[#FFE866] to-[#CFAE00] bg-clip-text text-transparent drop-shadow-md">Thought Partner</span>
 </h2>
 ...
 {/* Tabs */}
 <div className="flex border-b border-navy/5" role="tablist" aria-label="AI Copilot views">
 {TABS.map((tab) => (
 <button
 key={tab.id}
 id={`tab-${tab.id}`}
 ...
 msg.role === 'user'
 ? 'bg-white/5/8 border border-white/10 text-slate-700'
 : 'bg-depth-2 border border-navy/5 text-slate-300'
 }`}
 >
 {msg.text}
 </div>
 ...
 <div className="h-2 rounded-full bg-[#0A1E3A]/5">
 <motion.div
 initial={{ width: 0 }}
 ...
 className="flex gap-4 surface-base border-subtle rounded-2xl p-5 hover:bg-[#0A1E3A]/5 transition-colors duration-300"
 >
 <div className={`h-10 w-10 rounded-xl bg-[#0A1E3A]/5 border border-white/8 flex items-center justify-center shrink-0`}>
