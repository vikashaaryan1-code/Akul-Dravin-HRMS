...
// ── OKR Card ──────────────────────────────────────────────────────────────────
function OkrCard({ okr }: { okr: OkrItem }) {
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <p className="text-sm font-bold text-white leading-snug">{okr.objective}</p>
          ...
          <div className="space-y-2">
            {okr.keyResults.map((kr, i) => (
              <div key={i} className="flex items-center gap-2.5">
                ...
                <span className={`text-xs ${kr.done ? 'text-slate-400 line-through' : 'text-white'}`}>{kr.label}</span>
              </div>
            ))}
          </div>
        </div>
    ...
    <section aria-labelledby="performance-heading">
      ...
      <div className="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Q2 2026 · 4 objectives · 12 key results tracked</p>
        </div>
