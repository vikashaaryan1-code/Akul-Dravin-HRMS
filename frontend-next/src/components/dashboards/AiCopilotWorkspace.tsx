...
const AGENT_SUGGESTIONS: Record<AgentMode, string[]> = {
  executive: [
...
function ActionModal({ action, onConfirm, onDismiss }: {
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md surface-raised border-subtle rounded-2xl p-6 space-y-4">
        ...
        <div className="flex gap-3 pt-1">
          <button
            onClick={onDismiss}
            className="flex-1 py-2.5 rounded-xl border border-white/8 bg-white/5 text-xs font-black text-slate-400 hover:text-white transition-colors"
          >
            Cancel
          </button>
          <button
...
// ── Message Bubble ────────────────────────────────────────────────────────────
function MessageBubble({ msg, accentColor }: { msg: StreamMessage; accentColor: string }) {
        ...
        <div className={`flex-1 max-w-[85%] ${isUser ? 'items-end' : 'items-start'} flex flex-col gap-2`}>
          <div
            className={`rounded-2xl px-4 py-3 text-sm leading-relaxed ${
              isUser
                ? 'bg-white/8 border border-white/8 text-slate-200 ml-auto'
                : 'bg-[#0D0D14] border border-white/[0.05] text-slate-300'
            }`}
          >
            {isUser ? (
              msg.content
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-white/8 bg-white/[0.04] text-xs font-black text-slate-300 hover:text-white transition-colors"
            >
            ...
            <div className="grid gap-2 w-full max-w-md">
              {AGENT_SUGGESTIONS[agentMode].map((suggestion) => (
                <button
                  ...
                  className="px-4 py-3 rounded-xl border border-white/8 bg-white/[0.03] text-xs text-slate-400 hover:text-white hover:bg-white/[0.05] transition-colors text-left"
                >
                  {suggestion}
                </button>
              ))}
