...
// ── Headcount Trend Chart wired to API ───────────────────────────────────────
function LiveHeadcountChart() {
...
// ── Dept Breakdown Donut wired to API ─────────────────────────────────────────
function LiveDeptDonut() {
...
// ── AI Insight Feed wired to API ──────────────────────────────────────────────
function LiveAiInsightFeed() {
...
// ── Pending Approvals Panel wired to API ─────────────────────────────────────
function LivePendingApprovalsPanel() {
...
export function ExecutiveCommandCenter() {
  ...
  // Keka Interactive State
  const [isPunchedIn, setIsPunchedIn] = React.useState(false);
  const [punchMessage, setPunchMessage] = React.useState<string | null>(null);

  // Razorpay Interactive State
  const [payoutStatus, setPayoutStatus] = React.useState<'idle' | 'processing' | 'completed'>('idle');
  ...
  return (
    <section aria-labelledby="command-center-heading">
              ...
              <GlassCard className="p-8 backdrop-blur-3xl bg-[#091a30]/40 border-white/[0.08] hover:border-sky-500/20 transition-all duration-300">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-slate-400 text-xs mt-1">Multi-country contractor management & compliant onboarding</p>
                  </div>
                  <button id="btn-create-contract" className="px-5 py-2.5 rounded-full bg-gradient-to-r from-sky-500 to-indigo-600 text-white font-bold text-xs shadow-lg hover:scale-105 transition-transform flex items-center gap-1">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={i} className="p-4 rounded-2xl bg-white/5 border border-white/5 flex flex-wrap items-center justify-between gap-4 hover:bg-white/10 transition-colors">
                      <div>
                        <p className="text-sm font-bold text-white">{contractor.name}</p>
                        <p className="text-xs text-slate-400 mt-0.5">{contractor.role} · <span className="text-sky-300">{contractor.country}</span></p>
                      </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-1 space-y-6">
              ...
              <GlassCard className="p-8 backdrop-blur-3xl bg-[#091a30]/40 border-white/[0.08] text-center hover:border-emerald-500/20 transition-all duration-300">
                ...
                <p className="text-slate-400 text-xs mt-1">General Shift: 09:00 AM - 06:00 PM</p>
              ...
              <GlassCard className="p-8 backdrop-blur-3xl bg-[#091a30]/40 border-white/[0.08]">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-slate-400 text-xs mt-1">Shift logs, leaves ledger, and compliance dashboard</p>
                  </div>
                </div>

                <div className="grid sm:grid-cols-3 gap-4 mb-6">
                  <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
                    <p className="text-slate-400 text-[10px] uppercase font-black">Present Today</p>
                    <p className="text-2xl font-bold text-emerald-400 mt-1">942 <span className="text-xs text-slate-500">/ 1247</span></p>
                  </div>
                  <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
                    <p className="text-slate-400 text-[10px] uppercase font-black">Late Arrivals</p>
                    <p className="text-2xl font-bold text-amber-400 mt-1">18 <span className="text-xs text-slate-500">within 15m</span></p>
                  </div>
                  <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
                    <p className="text-slate-400 text-[10px] uppercase font-black">On Paid Leave</p>
                    <p className="text-2xl font-bold text-sky-400 mt-1">5 <span className="text-xs text-slate-500">approved</span></p>
                  </div>
                </div>

                {/* Shift Schedule Table */}
                <h4 className="text-sm font-bold text-white mb-3">Shift & Geofencing Matrix</h4>
                    ...
                    <tbody>
                      {[
                        ...
                        <tr key={i} className="border-b border-white/[0.03] hover:bg-white/[0.01]">
                          <td className="py-3 font-bold text-white">{item.dept}</td>
              ...
              <GlassCard className="p-8 backdrop-blur-3xl bg-[#091a30]/40 border-white/[0.08] hover:border-blue-500/20 transition-all duration-300">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-slate-400 text-xs mt-1">One-click auto-disbursements & Indian tax slab compliance</p>
                  </div>
                  <div className="flex gap-2">
                    {payoutStatus === 'completed' && (
                      <button
                        ...
                        className="px-4 py-2 rounded-full border border-white/10 hover:bg-white/5 text-xs text-slate-300 font-bold transition-colors"
                      >
                        Reset Status
                      </button>
                    )}
                    <button
                      ...
                      className="px-5 py-2.5 rounded-full bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-bold text-xs shadow-lg hover:scale-105 transition-transform disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                    >
                ...
                <h4 className="text-sm font-bold text-white mb-3">Instant Disbursement Queue</h4>
                <div className="space-y-3">
                  {[
                    { item: 'Salary Batch (HDFC Corporate Route)', amount: '₹3,10,42,800', type: 'Instant Bank Payout', status: payoutStatus === 'completed' ? 'Disbursed' : payoutStatus === 'processing' ? 'Processing' : 'Approved', color: payoutStatus === 'completed' ? 'text-emerald-400 border-emerald-500/20 bg-emerald-500/5' : payoutStatus === 'processing' ? 'text-blue-400 border-blue-500/20 bg-blue-500/5 animate-pulse' : 'text-slate-400 border-white/5 bg-white/5' },
                    { item: 'Contractor Invoices (ICICI Bank Direct)', amount: '₹81,20,000', type: 'Fast Settlement', status: payoutStatus === 'completed' ? 'Disbursed' : payoutStatus === 'processing' ? 'Processing' : 'Approved', color: payoutStatus === 'completed' ? 'text-emerald-400 border-emerald-500/20 bg-emerald-500/5' : payoutStatus === 'processing' ? 'text-blue-400 border-blue-500/20 bg-blue-500/5 animate-pulse' : 'text-slate-400 border-white/5 bg-white/5' },
                    ...
                    <div key={i} className={`p-4 rounded-2xl border flex flex-wrap items-center justify-between gap-4 transition-colors ${queue.color}`}>
                      <div>
                        <p className="text-xs font-bold text-white">{queue.item}</p>
            ...
            <div className="space-y-6">
              ...
              <GlassCard className="p-6 bg-gradient-to-br from-blue-900/20 to-slate-900 border-blue-500/20">
                ...
                <div className="mt-2 flex items-center justify-between text-xs text-slate-400">
                  ...
                  <span className="font-bold text-white">₹6.20 Cr</span>
                </div>
              </GlassCard>
            </div>
          </div>
      ...
      >
        {[
          { icon: Building2,   label: '47 Modules Active',   color: 'text-slate-400' },
