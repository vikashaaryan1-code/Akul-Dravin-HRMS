...
const PRIORITY_STYLE = {
  ...
  low:    'bg-slate-800 text-slate-500 border-white/8',
};
          ...
          <tbody>
            {filtered.map((c) => (
              <tr key={c.id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors group">
                ...
                <td className="py-3 pr-3 text-slate-400">{c.role}</td>
                <td className="py-3 pr-3">
                  <div className="flex items-center gap-2">
                    <span className={`font-black ${c.score >= 90 ? 'text-jade' : c.score >= 80 ? 'text-gold' : 'text-slate-400'}`}>{c.score}</span>
    ...
    <section aria-labelledby="ats-heading">
      ...
      <div className="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">38 open roles · 1,482 active applicants · 22d avg time-to-hire</p>
        </div>
