...
export function FormModal({
 ...
 >
 <div
 className={`relative w-full ${maxWidthMap[maxWidth]} rounded-2xl border border-slate-200/80 bg-white shadow-2xl animate-rise overflow-hidden`}
 >
 {/* Header */}
 ...
 <button
 onClick={onClose}
 className="ml-4 p-1.5 rounded-lg hover:bg-slate-100 text-slate-500 hover:text-slate-600 transition"
 >
 <X className="h-4 w-4" />
 </button>
 </div>
...
export function ModalInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
 ...
 <input
 {...props}
 className={`w-full px-3 py-2 text-sm rounded-xl border border-slate-200 bg-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30 transition ${props.className ?? ''}`}
 />
 );
}
...
export function ModalSelect(props: React.SelectHTMLAttributes<HTMLSelectElement> & { children: React.ReactNode }) {
 ...
 <select
 {...props}
 className={`w-full px-3 py-2 text-sm rounded-xl border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/30 transition ${props.className ?? ''}`}
 />
 );
}
...
export function ModalTextarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
 ...
 {...props}
 rows={props.rows ?? 3}
 className={`w-full px-3 py-2 text-sm rounded-xl border border-slate-200 bg-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30 resize-none transition ${props.className ?? ''}`}
 />
 );
}
...
export function SecondaryButton(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
 ...
 <button
 {...props}
 className={`px-4 py-2 text-sm font-medium rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 transition ${props.className ?? ''}`}
 />
 );
}
