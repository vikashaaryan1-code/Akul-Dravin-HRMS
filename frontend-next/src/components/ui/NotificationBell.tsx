...
export function NotificationBell() {
 ...
 <div className="absolute right-0 mt-2 w-96 z-50 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden animate-rise">
 {/* Header */}
 ...
 {unreadCount > 0 && (
 <span className="px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-600 text-xs font-semibold">{unreadCount}</span>
 )}
 </div>
 ...
 >
 <span className="mt-0.5 p-1.5 rounded-lg bg-white shadow-sm shrink-0">{typeIcon(notif.type)}</span>
 <div className="flex-1 min-w-0">
 <p className={`text-xs font-semibold truncate ${notif.status === 'Unread' ? 'text-slate-800 ' : 'text-slate-600 '}`}>{notif.title}</p>
 ...
 className="opacity-0 group-hover:opacity-100 p-0.5 rounded text-slate-600 hover:text-slate-500 transition"
 >
 <X className="h-3 w-3" />
 </button>
 </div>
