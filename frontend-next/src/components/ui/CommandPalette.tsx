...
function highlight(text: string, query: string): React.ReactNode {
 ...
 return <>{text.slice(0, idx)}<mark className="bg-blue-100 text-blue-700 rounded px-0.5">{text.slice(idx, idx + query.length)}</mark>{text.slice(idx + query.length)}</>;
}
...
export function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
 ...
 <div
 className="relative w-full max-w-xl mx-4 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-rise"
 ...
 />
 <kbd className="hidden sm:flex px-1.5 py-0.5 rounded text-xs font-mono bg-slate-100 text-slate-500">ESC</kbd>
 </div>
 ...
 <div key={category}>
 <p className="px-4 py-1.5 text-xs font-semibold text-slate-500 uppercase tracking-widest">{category}</p>
 {items.map((cmd) => {
 ...
 className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition ${isSelected ? 'bg-blue-50 ' : 'hover:bg-slate-50 '}`}
 >
 <span className={`p-1.5 rounded-lg ${isSelected ? 'bg-blue-100 text-blue-600 ' : 'bg-slate-100 text-slate-500'}`}>
 {cmd.icon}
 </span>
 <div className="flex-1 min-w-0">
 <p className="text-sm font-medium text-slate-800 ">{highlight(cmd.label, query)}</p>
