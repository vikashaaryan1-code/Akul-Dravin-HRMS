...
/* ── Nav Group Section ───────────────────────────────────────────────────────── */ function NavGroupSection({
 ...
 type="button"
 onClick={() => setOpen((v) => !v)}
 className="flex w-full items-center justify-between px-2 py-1.5 text-[9px] font-black uppercase tracking-[0.2em] text-slate-500 hover:text-slate-500 transition-colors"
 >
 {group.label}
 {open
...
/* ── SideNavigation ──────────────────────────────────────────────────────────── */ export function SideNavigation({ activeRole, isOpen, onClose }: SideNavigationProps) {
 ...
 <div
 className={clsx(
 'fixed inset-0 z-30 bg-slate-50/50 backdrop-blur-sm transition-opacity lg:hidden',
 ...
 <aside
 className={clsx(
 'fixed left-0 top-0 z-40 flex h-full w-72 flex-col border-r border-white/10 bg-white/98 shadow-2xl glass-3d-panel !rounded-none !border-y-0 !border-l-0 transition-transform duration-300 lg:sticky lg:top-16 lg:z-10 lg:h-[calc(100vh-4rem)] lg:translate-x-0 lg:shadow-none ',
 ...
 className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-8 pr-3 text-xs text-slate-700 outline-none transition focus:border-slate-400 focus:bg-white "
 ...
 <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">Platform Status</p>
 </div>
 <p className="text-xs font-bold text-navy">{totalAccessible} modules accessible</p>
 <p className="mt-0.5 text-[10px] text-slate-500">Role: <span className="text-slate-500 font-semibold">{activeRole}</span></p>
 </div>
 </div>
 </aside>
 </>
