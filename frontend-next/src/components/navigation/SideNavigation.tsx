  ...
  return (
    <div>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between px-2 py-1.5 text-[9px] font-black uppercase tracking-[0.2em] text-slate-500 hover:text-slate-400 dark:text-slate-600 dark:hover:text-slate-400 transition-colors"
      >
        {group.label}
        {open
          ...
          {accessibleItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {active && (
                  <span className="ml-auto h-1.5 w-1.5 rounded-full bg-emerald-400 dark:bg-emerald-500 shrink-0" />
                )}
              </Link>
            );
          })}
...
export function SideNavigation({ activeRole, isOpen, onClose }: SideNavigationProps) {
  ...
  // Count accessible items across all groups
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          'fixed left-0 top-0 z-40 flex h-full w-72 flex-col border-r border-white/10 bg-white/98 shadow-2xl glass-3d-panel !rounded-none !border-y-0 !border-l-0 transition-transform duration-300 lg:sticky lg:top-16 lg:z-10 lg:h-[calc(100vh-4rem)] lg:translate-x-0 lg:shadow-none dark:bg-slate-950/98',
          isOpen ? 'translate-x-0' : '-translate-x-full',
        )}
      >
        {/* Mobile header */}
        <div className="flex shrink-0 items-center justify-between border-b border-slate-200 px-4 py-3 lg:hidden dark:border-slate-800">
          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">Navigation</p>
          <button
            type="button"
            onClick={onClose}
            className="inline-flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-400 dark:hover:bg-slate-800"
        ...
        <div className="shrink-0 px-3 pt-3 pb-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-8 pr-3 text-xs text-slate-700 outline-none transition focus:border-slate-400 focus:bg-white dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200 dark:focus:border-slate-600 dark:focus:bg-slate-900/80"
        ...
        <div className="shrink-0 border-t border-slate-200 p-3 dark:border-slate-800">
          <div className="rounded-xl bg-gradient-to-br from-slate-900 to-slate-800 p-3 dark:from-slate-800 dark:to-slate-900">
            ...
            <p className="mt-0.5 text-[10px] text-slate-500">Role: <span className="text-slate-400 font-semibold">{activeRole}</span></p>
          </div>
        </div>
      </aside>
    </>
