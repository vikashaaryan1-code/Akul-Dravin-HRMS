...
/* ── Password strength helpers ───────────────────────────────────────────────── */ const RULES = [
...
function ResetPasswordContent() {
 ...
 setSuccess(true);
 /* Auto-redirect to login after 3 seconds */ setTimeout(() => router.push('/login'), 3000);
 } catch (err) {
 ...
 /* ── Invalid token state ─────────────────────────────────────────────────── */ if (tokenValid === false) {
 return (
 ...
 <Link
 href="/login"
 className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-6 py-3 text-sm font-bold text-navy transition hover:bg-white/10"
 >
 Sign In Now
 <ArrowRight className="h-4 w-4" />
 </Link>
